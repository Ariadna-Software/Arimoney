...
On Error GoTo EcomprobarCuentasBancariasRecibos
    ...
    SQL = "select codmacta,numserie,codfaccl ,codbanco,codsucur,cuentaba,digcontr,iban,bic from scobro left join sbic on codbanco=entidad  "
    SQL = SQL & " where codrem = " & RecuperaValor(Remesa, 1) & " AND anyorem=" & RecuperaValor(Remesa, 2)
    ...
    While Not miRsAux.EOF
                ...
                If SQL <> CC Then
                    ...
                    SQL = "CC. calculado:" & SQL & "   [" & Format(DBLet(miRsAux!digcontr, "N"), "00") & "]"
                    SQL = miRsAux!codmacta & " FRA: " & miRsAux!NUmSerie & miRsAux!codfaccl & "   " & SQL
                    Registro = Registro & SQL & vbCrLf
                    
                    
                Else
                    'MARZO 2016. Veremos si tiene BIC y si elIBAN calculado es igual al que lleva
                    If IsNull(miRsAux!BIC) Then
                        SQL = " --> BIC no existe"
                        SQL = miRsAux!codmacta & " FRA: " & miRsAux!NUmSerie & miRsAux!codfaccl & "   " & miRsAux!codbanco & ": " & SQL
                        Registro = Registro & SQL & vbCrLf
                    Else
                        'EL IBAN
                        ...
                        If Not DevuelveIBAN2("ES", AUX, SQL) Then
                            ...
                            SQL = " Calculando IBAN"
                        Else
                            If Mid(DBLet(miRsAux!IBAN, "T") & "XXXX", 3, 2) <> SQL Then
                                If DBLet(miRsAux!IBAN, "T") = "" Then
                                    SQL = "SIN IBAN "
                                Else
                                    SQL = "IBAN calculado: ES" & SQL & " [" & miRsAux!IBAN & "]"
                                End If
                            Else
                                'OK. Todo OK
                                SQL = ""
    ...
    If Registro <> "" Then
        SQL = "Las siguientes cuentas no son correctas. ¿Continuar?" & vbCrLf & Registro
