...
Private Sub MontaSQLAvisosFalta(ByRef SQ As String)
    ...
    SQ = " FROM scobro,cuentas WHERE scobro.codmacta = cuentas.codmacta AND cuentas.numpoliz<>'' and situacionjuri=0"
...
'    SQ = SQ & " AND fecfaccl >= '" & Format(vParam.FechaIniSeg, FormatoFecha) & "'"
    ...
    If vParam.FechaSeguroEsFra Then
        'ALZIRA
        SQ = SQ & " AND DATEDIFF(curdate(),fecfaccl) > " & vParam.DiasMaxAvisoD
    Else
        'HERBELCA
        SQ = SQ & " AND DATEDIFF(curdate(),fecvenci) > " & vParam.DiasMaxAvisoD
    End If
    
    'Mayo 2012
    SQ = SQ & " AND if(fecbajcre is null,True,fecfaccl<=fecbajcre)"
...
'    SQ = SQ & " AND fecfaccl >= '" & Format(vParam.FechaIniSeg, FormatoFecha) & "'"
    ...
    F = DateAdd("d", vParam.DiasMaxSiniestroD, Now)
    SQ = SQ & " and ( feccomunica<='" & Format(F, FormatoFecha) & "'"
    F = DateAdd("d", vParam.DiasAvisoDesdeProrroga, Now)
